...
function getSliceData(src, options) {
    ...
    var commentsData = {};
    var commentRe = /\/\*[\s\S]*?\*\//g;
    cssData = cssData.toString().replace(commentRe, function(a) {
...
function cssSpriteSmith(options, callback) {
    ...
    if(!options.spritepath) {
        options.spritepath = path.relative(path.dirname(src), options.spritedest);
    }
    ...
    async.waterfall([
        ...
        // process sprite data
        function processSpriteData(spriteImgData, cb) {
        ...
        function processRetinaSpriteData(retinaSpriteImgData, cb) {
